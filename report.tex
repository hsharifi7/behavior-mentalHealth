...
this relationship in an interactive visualization. It also analyzes the data and creates a model for predicting the possibility of 
having a mental health based on input on six factors. The research provides a more enhanced model by tuning and add/remove features. 
It proposes guidelines that could lead to reduction of depression and anxiety in the society. 
...
factors but it also used the state of art machine learning (ML) and deep learning (DL) algorithm to provide a statistical model for predicting 
...
\par\noindent\textit{$\circ$ The dataset BRFSS}\newline
...
We need more time to optimize our model(s) and increase the accuracy. We consider that a future work for this 
...
analysis. The statistical analysis and ML model build was done by each member and results reported and generated separately. 
...
into one, optimizing the ML model(s), and writing the reports and proof writing them. For more details on how tasks were distributed, please 
...
As part of this research, we examined multiple machine learning models to create a predictive model that by feeding the six factors,
it predicts the possibility of getting mental health. 
We evaluated six ML models, and examined multiple DL model layer setups.
The accuracy of our model(s) were very low at the beginning. Using hyper-parameters option from scikit-learn \cite{scikit} package, we managed to 
increase the accuracy to 69\%. We tried to use DL models for classifications using Keras \cite{keras} DL programming language. 
Though DL techniques are used in image recognition,
we build a DL classification model and after hours of trainings and model setup modification, we yet got 68\%.

To enhance our model, we took our research one step up and re-evaluated features we were using for our model. 
We dropped some features and added features. We managed to get the accuracy of our model to 72%.
At the end, Random Forest classifier performed the best for our analysis. Fig \ref{fig:project-features} shows the features used for our optimized model. 
We are not pleases with this accuracy but not disappointed either. We are planning to put more time and effort
as a follow up to this research to increase the accuracy to above 90\%.  
